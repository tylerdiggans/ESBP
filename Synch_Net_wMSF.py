...
def get_input():
	...
	parser.add_argument('--synchtol', default=1E-5, 
...
# Simulate Oscillator to obtain a point near attractor
	...
	X = odeint(Func, y0, range(T_trans), tfirst=True, args=(a,b,c), atol=1E-6, rtol=1E-6)
...
# Make it so the oscillators do not start out synchronized...
	...
	while cnt<nn:
		...
		X0 = S1 + np.array([local_random.normalvariate(0,3) for i in range(d*N)])
...
#Simulate until a single (3D) point is relaxed onto the attractor 
	T_trans, T = [100, 500] 	# Sim time for transit to attractor and synch
...
#  		Save these initial conditions in a txt file input by user for later reuse
	if X0s is None:
		if nn>10:
			...
			M = int(nn/10.)
			state = np.random.randint(2E10, size=M)
			pool = Pool(cores)
			...
			state = np.random.randint(2E10,size=nn)
			pool = Pool(cores)
